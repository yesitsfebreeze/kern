...
- `Queue.stats` is a single `Mutex<(completed_count, total_latency)>` (not two separate counters) so the hot `record_task_latency` path takes one mutex, not two.
- Deferred-task variants (`SeedQuestions`, `ClassifyContradiction`, `CommitAccess`) exist so the reason-LLM / access write-back never runs on the interactive ingest-commit or query path — the tick serializes that work off the hot path (see tasks.rs note for the measured motivation).
