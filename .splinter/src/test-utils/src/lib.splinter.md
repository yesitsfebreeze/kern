...
- Crate: test-only MCP infrastructure (`mcp_pipe`), scoped by being a dev-dependency. Kept in source: it is deliberately NOT `#![cfg(test)]` — that would compile to nothing for dependents' tests.
