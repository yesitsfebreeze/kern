...
# src/bench_support/build.rs — commentary (migrated from source doc comments)

- Module: benchmark graph construction — turns a replay `Trace`'s documents into a `GraphGnn` with similarity edges. The replay/scoring loop lives in `replay.rs`.
- `build_graph` populates the BM25 lexical index as the real load path does; without it, empty "hybrid" queries silently fall back to dense-only. (Tight hazard line kept inline; regression: `build_graph_populates_a_searchable_lexical_index`, originally filed for the empty-lexical-index bug.)
- `SIMILARITY_EDGE_FLOOR = 0.5`: cosine floor for a similarity edge. A looser floor densifies the graph and tanks ranking (NDCG) WITHOUT changing recall — a silent quality trap for anyone tuning it. (Tight hazard line kept inline.)
- `seed_similarity_edges` is O(n^2) ON PURPOSE. ANN top-k seeding was evaluated and REJECTED: it produces an identical edge set but is net slower. Do not "optimize" the O(n^2) scan away without re-running `pairwise_seeding_matches_ann_top_k_1k`. (Tight guard line kept inline.)
- Test fixtures: `thousand_doc_clustered_trace` = 1000 docs, 40 clusters of 3 near-identical docs (siblings cosine ~0.89 > FLOOR) among 880 unrelated singles → the only >FLOOR pairs are 120 edges (40 * C(3,2)). `ann_top_k_edge_ids` reproduces the edge set an ANN top-k seeder WOULD produce (NEIGHBOR_K=64, NEIGHBOR_EF=256), applying the same exact cosine + floor as the pairwise scan, to prove equivalence.
