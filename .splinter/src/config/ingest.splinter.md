...
[ingest] is the serde twin of the runtime ingest::Config; both default to the shared INGEST_* constants in base::constants. dedup_threshold: cosine floor in [0,1] — a nearest neighbour at or above this is merged as a duplicate instead of inserted (higher -> fewer merges). validate() delegates to the runtime ingest::Config check so the two config layers can never validate differently.
