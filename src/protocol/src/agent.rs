...
mod tests {
	...
	fn fork_kind_uses_adjacent_tag_content_tagging() {
		...
		let v: serde_json::Value = serde_json::from_str(
			&serde_json::to_string(&ForkKind::FileHandle {
				path: "p.rs".into(),
			})
			.unwrap(),
		)
		...
		assert!(
			vc.get("c").is_none(),
			"a unit variant has no content payload"
		);
	}
	...
	fn usage_summary_and_turn_result_roundtrip() {
		...
		let back: UsageSummary = serde_json::from_str(&serde_json::to_string(&usage).unwrap()).unwrap();
		...
		let ok_back: TurnResult = serde_json::from_str(&serde_json::to_string(&ok).unwrap()).unwrap();
		assert!(
			matches!(ok_back, TurnResult::Ok { reply, usage: Some(u) } if reply == "hi" && u.output_tokens == 34)
		);
		let err_back: TurnResult =
			serde_json::from_str(&serde_json::to_string(&TurnResult::Err("boom".into())).unwrap())
				.unwrap();
