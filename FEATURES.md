...
```
session delta (.txt) ──► capture intake ──► distill (LLM) ──► typed claims
                                                               │
...
- `Entity` (`src/base/types.rs:246`) — typed (`Normal`/`Fact`/`Document`/
  `Question`/`Claim`/`ReasonRef`), weighted by confidence (a beta distribution:
  `conf_mean`/`conf_variance`, updated via `observe_support`/`observe_contradict`)
...
- `Reason` (`src/base/types.rs:408`) — an edge `from`→`to` with a `kind`
  (`Similarity`/`Provenance`/`Spawn`/`Question`/`Supersedes`/`Contradiction`/
  `Related`), its own vector (mean of endpoints), a traversal `access_count`
  GCounter, and a CRDT `score`. `is_enriched`/`is_remote` flags.
...
**Where.** `src/base/types.rs` (830 LoC), `src/base/graph.rs` (1106 LoC),
...
**How** (`src/base/accept.rs:19` `accept()`):

1. **Dedup** — graph-wide top-1 vector search; if `score > DEFAULT_DEDUP_THRESHOLD`
   (0.92, `DEDUP_EF=64`), the thought is a duplicate and merges into the
...
**Where.** `src/base/accept.rs` (1178 LoC). Radii defaults in `constants.rs`
...
- `supersede_by_contradiction` (`src/base/accept.rs:380`) — inserts the new
...
- Classification is LLM-driven (`classify_prompt` / `parse_contradiction`,
  `accept.rs:360`) and **fails open to `Related`** (co-exist) — the
...
**Stages** (`retrieve_profiled`, `src/retrieval/answer.rs:134`, each checkpoint
...
| 2 | **Seed lexical** | `retrieval/seed.rs:100` | BM25 (`LexicalIndex`) candidate list, fused via RRF when `mode==Hybrid`. |
...
| 7 | **Boosts** | `retrieval/score.rs:77` | `apply_boosts`: confidence × score + **QBST** access/recency boost (`qbst`, capped at 0.1, 24h half-life) + `fact_score_boost` (0.3) for Facts. |
...
| 8 | **Filter** | `retrieval/score.rs:91` | Drop superseded; floor at `MIN_DELIVER_SCORE=0.40`; cap at `MAX_DELIVER_RESULTS=10` (MMR keeps a larger pool when on). Apply query options (source/kind/time/min_conf). |
...
**Where.** `src/retrieval/*` (4081 LoC, 12 files). Entry: `retrieval::query`
...
- **HNSW** (`src/base/hnsw.rs`, 1082 LoC) — id-stable, deterministic-build
  graph ANN. `insert`/`delete`/`search`/`search_batch`/`search_adaptive`
  (`AdaptiveEfConfig` scales `ef` with result size) / `search_filtered`
...
**How.** `QuantizationMode` (`Float`/`Int8`, `src/quant.rs`),
...
## 8. Capture & distillation (self-learning) — `active`

**What.** A conversation delta (`.txt`) dropped in `.kern/capture/` is drained,
...
- **Intake** (`src/ingest/intake.rs`) — `run()` polls `.kern/capture/`,
...
- **Place / split / direct** — `place.rs` builds chunk `Entity`s
  ...
  (LLM-assisted when given), `direct.rs` handles `.kern/direct/` synchronous
...
- **Outcome** (`src/ingest/outcome.rs`) — `OutcomeStatus` (Pending/Ok/Failed),
  `FailureReport::document_permanent` for non-retryable errors.

**Where.** `src/ingest/*` (2673 LoC, 12 files). Spawned by `spawn_capture`
(`src/commands.rs:805`).
...
- **Driver** (`tick::start`, `src/tick.rs:35`) — one async task drains the
...
- **Pulse** (`src/tick/pulse.rs`) — `pulse_with_half_life` re-deposits heat
...
**Where.** `src/tick/*` (2378 LoC, 6 files) + `src/tick.rs`.
...
**What.** Cold, stale, non-durable thoughts evict themselves; **Facts are
immune**; evictions spill to the cold tier before dropping (spill-before-drop
— eviction never loses data).

**How.** `stigmergy::run_gc` (`src/tick/stigmergy.rs:28`) collects victims per
kern where `is_cold_victim` holds (heat below `COLD_HEAT_THRESHOLD=0.01` *and*
not accessed within `COLD_GC_AGE = 7 days` *and* `kind != Fact`), spills each
...
- `daemon` / `run_server` (`commands.rs:650`) — boots the full runtime: loads
  graph, spawns watchdog, LLM keepalive, file watcher, capture intake, gossip,
...
- **CRDTs** (`src/crdt.rs`, 366 LoC) — `GCounter`, `PnCounter`, `LwwRegister<T>`
  (lamport + producer tiebreak), `OrSet<T>` (add-tombstone). Applied to five
  `CrdtTarget`s (`src/gossip/types.rs`): `ThoughtAccessCount` (GCounter),
  `ReasonTraversalCount` (GCounter), `ReasonScore` (LWW), `ValidUntil` (LWW),
  `Statements` (OR-Set). This replaces the old wall-clock max-join.
...
`network_id` nodes. Delta/Pulse/Question message kinds + the fetch RPC are
*handled on receipt* but the **Delta/Pulse/Question senders are Phase-1 stubs**
(`start_delta_flush` now sends; Pulse/Question senders partial). Federation
tuning at scale (batch size, push vs pull, anti-entropy) is open.
...
**Gaps.** No auth/crypto. No anti-entropy merkle/snapshot exchange. Senders
for Pulse/Question not wired. No backpressure on remote-id cap (drops new,
keeps known).
...
`Embed`, `Reason`, `Answer`, `Capture`, `Tick`, `Gossip`, `Gnn`, `Graph`,
...
**Gaps.** No config validation (bad dim/model fails at runtime). No env-var
override layer. Secrets (API keys) stored in plaintext TOML.
...
- **Trace** (`bench_support/trace.rs`) — JSON workload format: `docs[]` +
  `queries[]` (each with `expected_ids`, `mode`, optional `filter_kind`).
- **build_graph** (`build.rs`) — materialize a `GraphGnn` from a trace.
- **replay** (`replay.rs`) — run all queries, report per-query hit/miss +
  `ReplayReport`.
- **compare** (`compare.rs`) — A/B multiple `VectorBackend`s on a synthetic
  `Corpus`.
- **sweep** (`sweep.rs`) — grid-search one `SweepParam` (e.g. ef, k1) and
  emit CSV.
...
- **log** (`src/log/` workspace crate), **config-io** (`src/config-io/`),
  **test-utils** (`src/test-utils/`) — shared workspace helpers.
