...
- A **capture delta** (a `.txt` file written to `<cwd>/.kern/capture/` by your
...
- The daemon keeps a **recall digest** (`retrieval::digest`) fresh at
  `<cwd>/.kern/digest.md` — the root's gravitons plus the hottest distilled
  thoughts. Clients read it into each new session. For mid-session deep recall,
  the model calls the `query` MCP tool directly.

Capture is best-effort: if the daemon or its LLM is down, the session proceeds
...
kern is agent-agnostic: capture is a `.txt` delta dropped in
`<cwd>/.kern/capture/`, and recall is `<cwd>/.kern/digest.md` plus the `query`
MCP tool (see the README's *Capture & recall* section). Both no-op in any
