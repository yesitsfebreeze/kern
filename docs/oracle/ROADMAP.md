...
`degradation_lines` (`:122`) beside `tick_health_lines` (`:175`). `HealthRes`
...
(`src/commands/admin.rs:128`), not merged: a daemon is serving the store, so its
...
`FEATURES.md:679-687`. **Mostly closed 2026-07-22, and deliberately left open —
...
(`src/base/health.rs:85`, `src/mcp.rs:146`, `src/commands/admin.rs:169`). The
one producer that must not be refused waits instead — `submit` awaits capacity
(`src/ingest/worker.rs:190`) and the file-watcher sink still calls it
...
from the *tick* queue, which is bounded at 512 (`FEATURES.md:439-440`).
...
(`src/llm.rs:764`) — the shape `src/ingest/worker.rs:529` established, which
...
`Worker::queue_depth` (`src/ingest/worker.rs:181`) reads the mpsc channel's own
...
it live off the serving worker's channel (`src/mcp.rs:149`) rather than off a
...
only (`ingest_health_lines`, `src/commands/admin.rs:201`) — no daemon, no line,
...
(`src/ingest/worker.rs:593`), and `tests/e2e/test_health_surface.py` parks
...
pooling either (`FEATURES.md:981-982`) — that one is not gossip and is not gated
on 33.
...
No semantic or structural features (`FEATURES.md:504`), and naming plus
...
`FEATURES.md:600` for three claims that span 589-590). That is the same trade the
...
- ~~Watcher `.gitignore` parsing is approximate; no rename tracking~~ **(retired
  ...
  **a rename is not re-keyed in the graph.** `build_record`
  ...
  (`FEATURES.md:1090-1093`).
- `unnamed` lists only; there is no `promote` (`FEATURES.md:823`).
...
- Under WSL2 NAT a loopback Ollama URL must be hand-pinned; kern neither rewrites
  nor warns (`FEATURES.md:1135-1137`).
