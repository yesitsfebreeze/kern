...
- **Access.** When a query returns an entity, the retrieval path stamps it:
  ...
  (`stamp_access`, `src/retrieval/score.rs:217`). The default access deposit is
  `1.0`.
...
under the `[heat]` section (`src/config/mod.rs:48`).
...
The periodic driver is `spawn_maintenance_tick` (`src/commands.rs:942`). Every
...
`do_cluster` (`src/tick.rs:77`) is the structural half of compaction. For one
...
(`select_spawn_clusters`, `src/tick.rs:140`):
...
(`src/tick.rs:217`) folds any unnamed, empty child back into the parent and
...
child-first merely duplicates them briefly (`src/tick.rs:127`).
...
`run_gc` (`src/tick/stigmergy.rs:41`) never calls `remove_entity` on the
...
victim is simply removed (`src/tick/stigmergy.rs:66`). That is the intended
...
from confidence and `Fact` needs a full `1.0` (`src/base/math.rs:206`), which
...
- [Configure](../howto/configure.mdx) — the `[heat]` and `[tick]` knobs.
