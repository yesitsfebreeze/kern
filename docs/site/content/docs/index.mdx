---
title: "kern"
---

**A self-learning memory substrate for AI agents.** One long-running daemon per
working directory owns a knowledge graph that structures the durable facts you
hand it, keeps itself small without manual gardening, and serves the right
context back when you need it.

```
MCP ingest ─────────────────────► typed claims ─┐
.kern/intake/ drop → distill (LLM) → typed claims ┴→ graph → recall
```

Two caller-driven ways in — an agent calls MCP `ingest`, or a transcript is
dropped into `.kern/intake/` for the daemon to distill. kern captures no session
automatically.

## Where to start

- **New here?** [Why kern exists](./concepts/why-kern.mdx) — the problem, and why
  chunk-embed-retrieve does not solve it.
- **Want it running?** [Install & run the daemon](./howto/install-run.mdx), then
  [The memory bank](./howto/memory-bank.mdx).
- **Want the whole system in one diagram?** [Architecture](./concepts/architecture.mdx).

**Concepts** explain the mental model — read these to understand what kern is
doing and why. **How-to** pages are task-shaped: you have a goal, they get you
there.

<Callout type="warn" title="Maturity">
Federation is `building`, not shipped, and its transport is currently
unauthenticated and unencrypted. See [Federation](./concepts/federation.mdx)
before enabling it.
</Callout>

## Beyond these pages

- **[Vision](https://github.com/yesitsfebreeze/kern/blob/master/docs/oracle/VISION.md)** — what kern is and why it exists.
- **[README](https://github.com/yesitsfebreeze/kern/blob/master/README.md)** — install, configure, and run it.
- **[Design decisions](./decisions/index.mdx)** — the models and trade-off
  analyses behind the self-organizing, federated design, now indexed on this
  site.
- **[Roadmap](https://github.com/yesitsfebreeze/kern/blob/master/docs/oracle/ROADMAP.md)** — the north star, the measured baseline, and every open work item.
- **API** — rustdoc for every crate (`cargo doc --open`).
