...
Seed with **example statements, one per line** — statements shaped like the
claims you want routed there. A multi-line seed is embedded per line and the
normalized mean becomes the pull vector (`seed_examples`/`mean_pool`,
`src/base/accept.rs`). This is measured, not stylistic: against
...
("Architectural and design decisions, the alternatives considered…") and
0.55–0.61 for the same examples embedded as one concatenated blob. A
single-line seed is embedded whole; keep any one line under the embedding
model's context window, since each line is one embed call with no chunking.
An optional `mass` (default `1.0`) makes a graviton pull harder: routing
compares `distance / mass`, so a graviton at `mass = 2.0` claims content from
