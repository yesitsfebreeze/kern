...
Knowledge flows in two caller-driven ways — an agent calls MCP `ingest`
directly, or a transcript is dropped into `.kern/intake/` for the daemon to
distill (shown below). There is no automatic session capture:

```
.kern/intake/ file → distill (LLM) → claims → graph → recall
```

- An **intake delta** (a `.txt` file written to `<cwd>/.kern/intake/` by your
  agent, a wrapper, or `kern ingest` — kern ships no writer of its own) holds
  the new conversation text (user prompts + assistant text only).
...
root routes matching memories into (anything unmatched lands in `generic`).
Distillation labels claims with a built-in set of kinds (`preference`,
`decision`, `project`, `fact`, `code-fact`, `reference`, `procedural`); register
extra ones with `claim_kind` if your material needs them.
...
- **Graph CRDT (implemented).** `base::merge` provides content-addressed,
  ...
  The ranking signals a peer could set are **stripped on the way in**
  (`src/base/merge.rs:20`): an entity entering a `remote-*` kern has its heat,
  access counts and confidence reset to neutral, so heat's max join is moot
  inbound and a compromised node cannot pin a poisoned claim hot or confident
  federation-wide. Heat and confidence are earned by local use only. Because ids are content hashes, existence is a set union.
- **Capped cold tier (implemented).** Stigmergy GC spills cold, abandoned,
  ...
  eventually age out — and every row dropped past the cap increments a
  `cold_evicted` counter on `health`, because that drop is unrecoverable and
  the count is its only trace. Recall reaches the tier two ways: `kern get <id>`
  rehydrates by id, and the `query` tool fills remaining result slots from a
  cosine search over the cold tier (marked `cold:true`) when the hot graph
...
- **Bi-temporal invalidation (implemented).** When a same-kind near-duplicate
  ...
  deleted, and the spilled row carries its temporal triple with it, so a
  cold-recovered revision still answers a point-in-time query correctly. The
  `query` tool adds `as_of` (return the revision whose validity window covered a
  past instant) and `include_history` (also return superseded revisions
  reachable from the active hits, marked `history:true`). The
